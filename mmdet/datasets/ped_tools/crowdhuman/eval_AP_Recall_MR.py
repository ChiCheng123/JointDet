...
class Database(object):
    ...
    def loadData(self, fpath, body_key=None, head_key=None, if_gt=True):
        if if_gt:
            ...
            for record in records.values():
                ...
                self.images[record["ID"]].clip_all_boader() # clip border for both gt and dt
    ...
    def compare(self, thres=0.5, matching=None):
        ...
        # self.scorelist = self.scorelist[:100000]
    ...
    def eval_MR(self, ref="CALTECH_-2"):
        ...
        tp, fp = 0.0, 0.0
        fppiX, fppiY, fppiScore = list(), list(), list()
        ...
        return MR, (fppiX, fppiY, fppiScore)
...
def _evaluate_predictions_on_crowdhuman(gt_path, dt_path, target_key="box", mode=0) -> dict:
    """
    Evaluate the coco results using COCOEval API.
    """
    database = Database(gt_path, dt_path, target_key, target_key, mode)
    database.compare()
    AP, recall, AP_stats = database.eval_AP()
    mMR, mMR_stats = database.eval_MR()
    return AP, mMR, recall, AP_stats, mMR_stats

if __name__ == "__main__":
    det_body = "results.bbox.json"
    gt_body = "annotation_val.odgt"

    eval_results_body = _evaluate_predictions_on_crowdhuman(gt_body, det_body, mode=0)
