...
class JointDetRDM(TwoStageDetector):
    """
    Implementation of JointDet v1, with RDM (Relationship Discriminating Module)
    """
    ...
    def __init__(self,
        ...
        if rdm is not None:
            rdm_train_cfg = train_cfg.rdm if train_cfg is not None else None
            rdm_test_cfg = test_cfg.rdm if test_cfg is not None else None
            rdm.update(train_cfg=rdm_train_cfg)
            rdm.update(test_cfg=rdm_test_cfg)
            rdm.pretrained = pretrained
            self.rdm = build_head(rdm)
    ...
    def forward_train(self,
        ...
        with torch.no_grad():
            head_bbox_post, _ = self.roi_head_kopf.simple_test(x, proposal_list_kopf, img_metas, rescale=False)
            body_bbox_post, body_bbox_pre = self.roi_head_body.simple_test(x, proposal_list_body, img_metas, rescale=False)


        # relationship discriminating module
        rdm_losses = self.rdm.forward_train(x, img_metas, head_bbox_post, body_bbox_post, body_bbox_pre,
                                            gt_bboxes, gt_labels, gt_bboxes_ignore, gt_masks, **kwargs)
        losses.update(rdm_losses)
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        ...
        kopf_test_results_rescale, _ = self.roi_head_kopf.simple_test(
                    x, proposal_list_kopf, img_metas, rescale=True)

        kopf_test_results, _ = self.roi_head_kopf.simple_test(
            x, proposal_list_kopf, img_metas, rescale=False)
        body_bbox_post, body_bbox_pre = self.roi_head_body.simple_test(
            x, proposal_list_body, img_metas, rescale=False)

        body_results, head_results = self.rdm.simple_test(x, img_metas, kopf_test_results, body_bbox_post, body_bbox_pre)

        # rescale results to original image scope
        body_results[0][0] = body_results[0][0][:, :5] / np.hstack((img_metas[0]['scale_factor'], [1.0]))
        head_results[0][0] = head_results[0][0][:, :5] / np.hstack((img_metas[0]['scale_factor'], [1.0]))
